...
- Open-ended game synthesis remains LLM-driven; greenfield scaffolding only creates a minimal Godot project shell.
...
- Interactive pending build approval with `/apply` and `/reject`; `/apply` writes the exact in-memory preview generated by the current session.
